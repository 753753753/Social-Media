...
import { clearUpdateStatus, fetchProfile, updateProfile } from "../../features/auth/profileSlice";
...
const EditProfile = () => {
  ...
  const { data: profile, loading, updating, updateError, updateSuccess } = useSelector(
    (state) => state.profile
  );
  ...
  // When profile data loads, set form values
  useEffect(() => {
    if (profile) {
      setForm({
  ...
  useEffect(() => {
    if (updateSuccess) {
      // Optionally, clear the update status after showing a toast
      toast.success("Profile updated successfully!");
      dispatch(clearUpdateStatus());
      navigate("/editprofile", { state: { showprofileToast: true } });
    }
    if (updateError) {
