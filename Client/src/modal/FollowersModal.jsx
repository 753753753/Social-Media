...
function FollowersModal({ isOpen, onClose, follower , followedUsers }) {
...
<div className="max-h-[300px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-800 mt-2 p-1">
    {follower && follower.length > 0 ? (
            ...
            return (
                ...
                >
                    ...
                    <img
                        ...
                        onClick={() => user?.followedUserId?._id && navigate(`/user/${user.followedUserId._id}`)}
                    />
                    ...
                    <div
                        ...
                        onClick={() => user?.followedUserId?._id && navigate(`/user/${user.followedUserId._id}`)}
                    >
                        <p className="font-semibold">{user?.followedUserId?.name || "Unknown User"}</p>
                    ...
                    <button
                        className={`px-4 py-1.5 transition cursor-pointer rounded-2xl ${
                            followedUsers[user?.followedUserId?._id]
                                ? "bg-white text-black border border-gray-300 hover:bg-gray-100"
                                : "bg-[#877EFF] text-white hover:bg-[#877EFF]"
                        }`}
                        onClick={() => user?.followedUserId?._id && handleFollow(user.followedUserId._id)}
                    >
        ...
        <p className="text-center text-gray-400 py-4">No followers</p>
    )}
</div>
